...
public class Projectile {
   final double ACCELERATION_Y = 980; // CM
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        /*while (true)
        {*/
            deltaX = cannon.prompt("∆x = ", "Error: choice out of range.", 0, true, 1000, true, true);
            cannon.fancyCalculation(500);
            if (deltaX > maxRange)
            {
            ...
            if (!isIterationOverrideEnabled)
            {
                ...
                if (cannon.prompt(ANSI_CYAN + "Was the attack a success?" + ANSI_RESET, "Error: invalid answer.", new String[]{"yes", "no"}).matches("yes"))
                {
                    greenGuys++;
    ...
    public String prompt(String message, String errorMessage, String[] keywords/*, Type inputType*/) // All-purpose prompter; inputType DISABLED rn; TODO: allow for bounds (e.g. only strings that start with s, or ints 1-10)
    {
        while (true)
        {
    ...
    public void fancyCalculation(long delay) throws InterruptedException {
    ...
    {
        return Math.pow(initialVelocity, 2) * (Math.sin(90) / ACCELERATION_Y); // CENTIMETERS
        // v0 = sqrt(Rg/sin(2Ø))
        // R = (v0)^2*sin(2Ø)/g
    }
